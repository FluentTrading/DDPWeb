...
public final class MatchManager{
    ...
    public final static GameResultManager packData( DDPMeta meta, Map<NFLTeam, NFLMatchInfo> scoreMap, DBService service ){
    ...
    public final static GameResult packData( DDPPick pick, Map<NFLTeam, NFLMatchInfo> scoreMap ){

        int index                       = 0;
        Set<String> uniqueTeamsSet      = new HashSet<>( );
        NFLTeam[] myPickedTeams         = pick.getTeams( );
        NFLMatchInfo[] myScores         = new NFLMatchInfo[ myPickedTeams.length ];
        
        for( NFLTeam myTeam : myPickedTeams ){
            
            NFLMatchInfo score      = scoreMap.get( myTeam );
            if( score != null ) {
